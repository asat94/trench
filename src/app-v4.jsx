...
function Market({ tokens, market, status, open, saved, setSaved, changePage }) {
  ...
  const highestVolume = Math.max(...chains.map((item) => item.vol));
  return <><div className="hero"><div><span className="eyebrow"><i /> MARKET PULSE</span><h1>The market, <em>at a glance.</em></h1><p>Live chain flow and the opportunities pulling attention.</p></div></div>
    ...
    {!tokens.length ? <section className="emptybox"><Radar size={28} /><h2>Live market data is unavailable</h2><p>{status}</p></section> : <section className="signalboard"><div className="signalmain"><div className="head"><div><span>HIGH-SIGNAL OPPORTUNITIES</span><h2>What’s pulling attention</h2><p>Live market activity, ranked by Trench Score.</p></div><button className="button ghost" onClick={() => changePage('Trench Radar')}>Open Radar <ArrowUpRight size={14} /></button></div><Table tokens={tokens.slice(0, 7)} open={open} saved={saved} setSaved={setSaved} /></div><aside className="signalside"><div className="head"><div><span>CHAIN PULSE</span><h2>Flow by chain</h2></div><ChevronDown size={15} /></div><div className="signalchains">{chains.map((item) => { const percent = total ? Math.round(item.vol / total * 100) : 0; return <div className="signalchain" key={item.chain}><span>{chainLabels[item.chain]}</span><div><b>{item.chain}</b><small>{fmt(item.vol)} volume</small></div><strong>{percent}%</strong><i><em style={{ width: `${percent}%` }} /></i>{item.vol === highestVolume && item.vol > 0 ? <Tag hot>HOT</Tag> : null}</div>; })}</div><div className="notable"><span>NEW & NOTABLE</span>{tokens.slice(0, 3).map((token) => <button key={`${token.chain}-${token.symbol}`} onClick={() => open(token)}><Coin token={token} /><div><b>{token.name}</b><small>{token.symbol} · {chainLabels[token.chain] || token.chain}</small></div><Change value={token.change} /><ArrowUpRight size={14} /></button>)}</div></aside></section>}</>;
}
