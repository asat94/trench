...
function Market({ tokens, market, status, open, saved, setSaved, changePage }) {
  const chains = market?.chainVolumes?.length ? market.chainVolumes : ['Solana', 'BNB Chain', 'Robinhood Chain', 'Base', 'Ethereum'].map((chain) => ({ chain, vol: tokens.filter((token) => token.chain === chain).reduce((sum, token) => sum + Number(token.vol || 0), 0) }));
