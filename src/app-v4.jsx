...
function TokenPanel({ token, close, saved, setSaved }) {
  ...
  return <div className="shade" onClick={close}><section className="drawer" onClick={(event) => event.stopPropagation()}>
    ...
    <div className="momentum"><div><span>24h price movement</span><b className={Number(token.change) >= 0 ? 'up' : 'down'}>{Number(token.change) >= 0 ? 'Upward' : 'Downward'} momentum</b></div><svg viewBox="0 0 280 64" role="img" aria-label="24 hour price movement"><defs><linearGradient id="trend" x1="0" x2="0" y1="0" y2="1"><stop stopColor={Number(token.change) >= 0 ? '#b6ff3b' : '#ff6658'} stopOpacity=".4"/><stop offset="1" stopColor="transparent"/></linearGradient></defs><path className={Number(token.change) >= 0 ? 'positive' : 'negative'} d={Number(token.change) >= 0 ? 'M2 50 C35 47 45 42 70 45 S105 25 132 34 S165 40 190 18 S230 22 278 7' : 'M2 12 C34 11 51 25 74 21 S111 29 136 36 S165 30 193 48 S232 45 278 57'} /><path className="area" fill="url(#trend)" d={Number(token.change) >= 0 ? 'M2 50 C35 47 45 42 70 45 S105 25 132 34 S165 40 190 18 S230 22 278 7 L278 64 L2 64 Z' : 'M2 12 C34 11 51 25 74 21 S111 29 136 36 S165 30 193 48 S232 45 278 57 L278 64 L2 64 Z'} /></svg></div>
...
function WalletPanel({ wallet, close }) {
  ...
  return <div className="shade" onClick={close}><section className="drawer walletdrawer" onClick={(event) => event.stopPropagation()}>
    ...
    {data?.error ? <p className="empty">{data.error}</p> : !profile ? <p className="empty">Loading wallet profile…</p> : <><div className="stats walletstats">{[['30d realised PnL', profile.realized], ['30d unrealised PnL', profile.unrealized], ['Win rate', profile.winRate], ['PnL ratio', profile.pnl], ['Buys / sells', `${profile.buys} / ${profile.sells}`]].map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div><div className="why"><h3>Why this wallet is here</h3><p>{profile.reason} These figures are research signals, not a guarantee of future results.</p></div><h3 className="activitytitle">Recent buys and sells</h3><div className="activitylist">{(data.activities || []).map((item) => <div key={item.id}><b className={item.type === 'BUY' ? 'up' : 'down'}>{item.type}</b><span>{item.symbol}<small>{item.amount} · {item.time}</small></span></div>)}</div></>}
  </section></div>;
}
